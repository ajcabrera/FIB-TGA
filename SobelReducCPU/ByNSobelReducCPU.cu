...
__global__ void KernelReduction(int NT,int *S, int *oMin, int *oMax) { //Reduction
...
__global__ void KernelSobel2 (int N,int M,unsigned char *A, int *S, int NS, float factor) {
    ...
    if(row < M && col < NS) {
        if(col != 0 && row != 0 && col != NS-1 && row != M-1)
            A[row*N+col*3] = A[row*N+col*3+1] = A[row*N+col*3+2] = (unsigned char)(S[row*NS+col] * factor);
...
int main(int argc, char** argv)
{
  ...
  float TiempoTotal, TiempoKernel, factor;
  ...
  int *hmax = (int*)malloc(numBytesR);
  int *hmin = (int*)malloc(numBytesR);
  ...
  KernelReduction<<<dimGridR,dimBlockR>>>(NS*M, d_S, d_OutMin, d_OutMax);

  cudaMemcpy(hmax, d_OutMax, numBytesR, cudaMemcpyDeviceToHost);
  cudaMemcpy(hmin, d_OutMin, numBytesR, cudaMemcpyDeviceToHost);

  int max = hmax[0];
  int min = hmin[0];

  for (int i=1; i < nBlocksR; i++) {
    if (hmax[i] > max) max = hmax[i];
    if (hmin[i] < min) min = hmin[i];
  }
  factor = (float)(255.0/(float)(max-min));

  KernelSobel2<<<dimGrid, dimBlock>>>(N, M, d_A, d_S, NS, factor);
  ...
  printf("nThreadsR: %dx%d (%d)\n", nThreads*nThreads, 1, nThreads * nThreads);
  printf("nBlocksR: %dx%d (%d)\n", nBlocksR, 1, nBlocksR);
